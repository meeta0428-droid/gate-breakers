import { Character, calculateDamageFromCards, calculateDefenseFromCards, executeCardEffects, triggerHook } from './game_logic_v9.js?v=411';
...
const els = {
    ...
    btnExportDeck: document.getElementById('btn-export-deck'),
            ...
            cardDiv.onclick = () => {
                ...
                if (c.name === 'ドリフトヴェイド') {
                    ...
                    if (enemyInitStr !== null) {
                        const cleanStr = enemyInitStr.replace(/[０-９]/g, s => String.fromCharCode(s.charCodeAt(0) - 0xFEE0));
                        const enemyInit = parseInt(cleanStr) || 0;
                ...
                cDiv.querySelector('button').addEventListener('click', () => {
                    ...
                    if (s.card.name === '彷徨う砂塵霊') {
                        ...
                        if (s.card.isChimera && s.card.originalCards) {
                            ...
                            s.card.originalCards.forEach(c => player.deck.void.push(c)); handleSummonVoided(player, s.card);
...
（合成されたユニットは1体として扱われ、コスト・攻撃・防御が合算されます。また1ラウンドの間に攻撃と防御を1回ずつ行えます）`);
                ...
                if (passiveCard.name === 'サラマンダー') {
                    ...
                    if (summonIdx > -1) {
                        ...
                        player.deck.summons.splice(summonIdx, 1);
                        player.deck.void.push(salamanderCard); handleSummonVoided(player, salamanderCard);
                ...
                if (passiveCard.name === 'ウンディーネ') {
                    ...
                    if (summonIdx > -1) {
                        ...
                        player.deck.summons.splice(summonIdx, 1);
                        player.deck.void.push(undineCard); handleSummonVoided(player, undineCard);
