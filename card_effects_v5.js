    ...
    "キュアライト": {
        onAttack: (context) => {
            if (context.player && !context._curelightApplied) {
                ...
                if (context.logMsg) {
                    context.logMsg(`【キュアライト】癒やしの光！現在の捨札は ${discardCount} 枚。<br>強度が <b>${finalStrength}</b> になりました！<br><span style="color:#ff66cc; font-weight:bold;">※対象の味方（または自分）は廃棄札から <b>コスト${finalStrength} 以下</b> のカードを1枚選び、手動で山札に戻してください。</span>`, 'important');
                }
